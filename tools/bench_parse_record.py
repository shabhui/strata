...
量到的差别就是这么来的(同一份合成输入,src 用 git stash 换掉再跑一遍):
    ...
    近道五六  在用位读一个字节(空闲记录连记录头都不建);fixup 不重读 usa_*

**近道五六只有比值,没有绝对值** —— 量的时候机器上有个 qemu 虚拟机在跑(3533 秒
CPU、2.36 GB),整机慢了一成半以上。那种情况下绝对值没有意义,所以改成两个版本
交错跑、只看比值:三轮、两种顺序,新版每次都赢,中位数 156,720 / 144,264 = 1.09x。
上面表里 169,207 那个数是机器空着的时候量的,别把它和这 1.09x 相乘当成新绝对值。
