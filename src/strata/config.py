...
#
# 两件事叠起来:MFT 当时既慢 2.6 倍又把总量说成两倍多。留着它当默认值,
# 等于让提权成功反过来把体验变差。
#
# 这两条理由现在都不成立了,所以默认值改回 True:
#
# 一、慢的问题解决了。缓冲区复用(mft.py)加页缓存开大(db.py)之后,同一台机器
#    背靠背各跑一次完整 scan_drive(tools/bench_paths_head2head.py,C:):
#
#      第一次   mft 平均 30.3s   scandir 平均 43.4s   MFT 快 1.43x
#      第二次   mft 平均 80.1s   scandir 平均 115.5s  MFT 快 1.44x
#
#    绝对值飘了 2~4 倍(机器负载、缓存冷热),但**比值稳在 1.43~1.44**。
...
# 不是逻辑大小。界面已经显示走的是哪条路(app.js:218 和 :1773),两次扫描
# 方法不同时也有专门的提示文案。
#
# 没提权的时候照旧退回 scandir —— collect_entries 捕 AccessDenied,note 里
# 会写明原因。改这个默认值只影响提权跑的那些次。
PREFER_MFT = True
